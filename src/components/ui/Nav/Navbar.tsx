...
const Nav: React.FC = () => {
  ...
  const menuOverlayBarRef = useRef<HTMLDivElement | null>(null);
  const menuOpenBtnRef = useRef<HTMLDivElement | null>(null);
  const menuCloseBtnRef = useRef<HTMLDivElement | null>(null);
  const menuFooterRef = useRef<HTMLDivElement | null>(null);

  function slideInOut(): void {
  ...
  const handleCloseMenu = () => {
    ...
    // Retrieve elements for GSAP animations
    const overlayBarAnchor = menuOverlayBarRef.current.querySelector(
      "a"
    ) as HTMLAnchorElement;
    const closeBtnImage = menuCloseBtnRef.current.querySelector(
      "img"
    ) as HTMLImageElement;
    const showreelAnchor = menuFooterRef.current.querySelector(
      ".showreel a"
    ) as HTMLAnchorElement;
    const mediaLinkAnchors = Array.from(
      menuFooterRef.current.querySelectorAll<HTMLAnchorElement>(".media-link a")
    );
    const navAnchor = navRef.current.querySelector("a") as HTMLAnchorElement;
    const menuOpenImage = menuOpenBtnRef.current.querySelector(
      "img"
    ) as HTMLImageElement;

    gsap.to(
      [overlayBarAnchor, closeBtnImage, showreelAnchor, ...mediaLinkAnchors],
      {
        y: -40,
        duration: 1,
        stagger: 0.1,
        ...
        onComplete: () => {
          gsap.set(overlayBarAnchor, { y: 20 });
          gsap.set(closeBtnImage, { y: 20 });
          gsap.set(showreelAnchor, { y: 20 });
          gsap.set(mediaLinkAnchors, { y: 20 });
        },
      }
    );
    ...
    gsap.to([navAnchor, menuOpenImage], {
      y: 0,
      duration: 1,
      stagger: 0.1,
      delay: 0.5,
  ...
  const handleOpenMenu = () => {
    ...
    gsap.to([navAnchor, menuOpenImage], {
      ...
      ease: CustomEase.create("", ".76,0,.2,1"),
      onComplete: () => {
        gsap.set(navAnchor, { y: 20 });
        gsap.set(menuOpenImage, { y: 20 });
      },
    });
  ...
  useEffect(() => {
    ...
    return () => {
      // Use the copied variables in cleanup
        ...
        <div className="menu-overlay" ref={menuOverlayRef}>
          ...
          <div className="menu-footer" ref={menuFooterRef}>
            <div className="media-link">
              <Link href="https://api.whatsapp.com/send?phone=971508190026">
                Whatsapp
              </Link>
            </div>
